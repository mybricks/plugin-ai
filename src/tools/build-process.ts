...
<关于MyBricks事件流程>
  ...
  **变量**
  变量是一个内置的特殊计算组件，是MyBricks平台中管理组件状态和数据的核心机制，其设计理念与React中的状态管理高度相似。理解变量何时使用、如何管理是流程搭建的关键。
  
  **何时用变量**
  请遵循以下决策原则：
  - **数据会发生变化**：当数据需要在应用生命周期内被更新、修改时（如计数器、表单输入、开关状态）
  - **需要响应式更新**：当数据变化需要自动触发UI更新时
  - **跨组件共享状态**：当同一份数据需要在多个组件间共享和同步时（如全局主题、用户信息）
  - **需要持久化或缓存**：当数据需要在不同操作间保持或缓存计算结果时
  
  **变量使用原则**
  - 语义唯一性原则：相同语义的数据应该使用同一个变量存储
  - 复用优先原则：优先复用已存在的变量，避免重复创建
  - 作用域匹配原则：变量必须在正确的作用域内创建和使用
  - 维护组件状态原则：如果组件的状态需要被其他组件读取或控制且本身不提供状态读取能力，就需要使用变量来实现状态的共享
...
- 当需要存储数据时，先判断该数据是"状态"还是"配置"：会变化的、需要共享的、需要响应式更新的属于状态，应使用变量；固定不变的、仅用于初始化的属于配置，不需要创建变量
- 决策变量绑定时，明确说明：该组件支持变量绑定（检查bindWith属性），且数据符合MVVM模式中的"模型"特征，因此采用变量绑定方案而非手动连接
</解释actions的调用过程>

<变量绑定方案>
  **变量绑定：MyBricks的MVVM实现**
  变量绑定是本平台实现MVVM架构的核心，通过声明式绑定替代命令式连接，让数据变化自动驱动视图更新。
  
  <MVVM模式对应关系>
  - **Model**：变量中存储的业务数据
  - **View**：UI组件构成的用户界面
  - **ViewModel**：变量与UI组件之间的绑定关系（通过bindWith配置）
  
  变量绑定的本质是建立Model与View之间的观察者模式：当Model（变量）变化时，自动通知所有观察者（绑定的UI组件）更新。
  </MVVM模式对应关系>
  
  <何时必须使用变量绑定>
  以下场景**必须使用变量绑定**，以实现真正的数据驱动：
  ...
  | 场景 | 传统连接方式 | 变量绑定方式 | 推荐选择 |
  |------|-------------|-------------|----------|
  | 表单数据收集 | 连接每个输入组件的onChange到变量set | 双向绑定变量 | **变量绑定**（减少90%连接） |
  | 多组件同步 | 连接变量到每个组件的输入端口 | 绑定同一变量 | **变量绑定**（一改全改） |
  | 简单展示 | 连接一次数据到展示组件 | 创建变量并绑定 | **传统方式**（更简单） |
  ...
  1. **结构化变量设计**
    ...
    // 绑定方式：分别绑定 userInfo.name, userInfo.score.chinese 等
    \`\`\`
  
  2. **分层状态管理**
    - 全局状态（如用户信息）：在根作用域创建
    - 页面状态（如筛选条件）：在页面作用域创建
    - 组件状态（如展开收起）：在组件内部作用域创建
...
- **MVVM优先原则**：当UI组件支持变量绑定时，优先考虑MVVM方案，检查是否可以通过绑定替代手动连接
- **响应式更新验证**：对于每个变量，思考"如果这个值变化，哪些UI需要更新？"确保所有依赖组件都已绑定
- **最小化原则**：只有当数据真正需要响应式更新或跨组件共享时，才为其创建变量
</思考建议>
...
["connectTo",output,input]`,
      ...
      注意：
       ...
       - 如果没有合适的组件实现事件，可以创建一个计算组件进行占位，并给出说明性标题
    </createEvent>
...
`,
  ...
  注意：
   - 当需求无法通过actions实现时，实事求是告诉用户，同时可以创建一个计算组件进行占位，并给出说明性标题。
</如何通过action搭建事件流程>
`
    },
    stream: (params: any) => {
