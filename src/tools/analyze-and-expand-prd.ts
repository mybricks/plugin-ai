  ...
  const {
    demo = {
      ...
      pages: [
        ...
        3. 轮播图，放置商品或者营销广告
        4. 猜你喜欢：展示推荐商品列表
         4.1 每个商品卡片包含商品图片、商品名称、商品价格、购买按钮
         4.2 不同商品类型可以有不同的异化卡片，例如今日最低价商品、秒杀商品、最热销商品等
        `,
        },
        {
          title: '分类',
        ...
        2. 分类模块
          ...
          2.2 商品列表
            2.2.1 商品卡片
            2.2.2 支持异化的卡片设计，例如促销商品、热销商品等`,
        },
        {
          title: '商家详情',
          prd: `目的：展示商家的详细信息，诱导用户进行下单购买
        需求：
        从上往下依次为
        1. 商家信息
        2. 商品列表
          2.1 商品卡片
          2.2 支持异化的卡片设计，例如促销商品、热销商品等`,
        },
        {
          title: '我的',
      ...
      style: `使用美团成熟的颜色系统是一个不错的选择
        ...
        - 圆角：12px，增加柔和感
        - 间距：16px
        同样的，你也可以参考一些常见的APP，比如美团和饿了么的元素设计。注意颜色不要太浅，也不要太鲜艳，按钮等比较大块的元素禁止使用点缀色`
    }
  } = config ?? {}
  return {
    name: NAME,
...
<处理流程>
  ...
  返回规则：
    - 如果是一句话需求，从上到下梳理并拓展需求，并整理成页面维度的需求
      ...
      - 如何拓展？
        - "一个简历页面" -> "一个个人简历页面，包含个人介绍（包含头像、名称、手机号、邮箱号、毕业院校等等具体详细的字段）、技能特长（具体到技能名称、熟练程度等）、项目经历（具体到项目名称、项目描述、项目时间等）、联系方式（具体到手机号还是电话还是邮箱）"
        - "实现用户管理系统" -> "
          一个用户管理系统，包含以下页面：
          - 登录页面
          - 用户列表页面
    ...
    - 如果需求较为详实，则整理之后，在满足用户需求的基础上，适当补充细节并整理成页面维度的需求
      ...
      - 如何合理补充细节，并整理成页面维度的需求？
        - 例如 "一个包含导航、公司介绍、公司优势、页脚的公司官网"
          - 可以整理为如下页面并补充整体框架：
            - 首页，包含以下内容：
              - "导航"
              - "公司介绍"
            ...
            - 联系我们页，包含一些内容：
              - "联系方式"
              - "在线留言"
          - 最终根据整理和拓展的需求，拆分成不同的页面，每个页面包含标题和自上而下的需求描述
