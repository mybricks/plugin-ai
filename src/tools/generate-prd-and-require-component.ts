...
按照以下步骤完成prd(需求分析规格说明书)文件：
  1、总体需求分析，详细分析用户需求，按以下格式进行整理<需求文档格式>
    *概要分析*[从顶级到叶子，分层级从上往下，从左到右，按照树状结构提取拆分需求中的事实信息，无需过于详细，同时告知这只是辅助信息]
    注意：对于区域的左右上下你是很清楚的，所以合理进行拆分，拆分错误会造成重大的后果，宁愿不拆分也不要拆分错误！！！
...
${mode === 'extract' ? `<examples>
  ...
  ${fileFormat({
    ...
    **概要分析**
    ...
    1. 顶层结构：顶部固定区域、内容区域、底部区域，从上到下垂直排列
    2. 顶部固定区域：通栏导航，包含左侧logo + 公告，右侧个人信息区域
      - 左侧logo + 公告信息（喇叭图标 + ”这是一条公告“）；
      - 右侧个人信息区域：蓝色边框的头像 + 昵称，提供退出登录按钮；
    3. 内容区域：左边自适应内容区域 + 右侧固定宽度的侧边栏
      - 左侧内容区域：从上到下包含筛选区域、推荐列表区域
        - 筛选区域：从上到下的分类标签(地区列表、物流列表、评分列表) + 右侧搜索按钮
        - 推荐列表区域：垂直一行一列的推荐卡片列表
          - 卡片内容：商品图片 + 价格（划线价 + 到手价）+ 补贴标签，右侧提供加购按钮
      - 右侧侧边栏区域：从上到下包含推荐热榜、个人信息区域
        - 推荐热榜：顶部的“大家都在搜” + “换一换”按钮，内容为各类商品的榜单列表，商品仅包含缩略图和到手价和实时热度小火苗；
        - 个人信息：包含浏览、收藏、关注等数据的数据卡片以及一个登录入口；
          - 数据卡片：核心指标 + 同比环比，以及上升下降的箭头；
    3. 底部区域：固定的底部内容，包含左侧的价格计算和右侧的支付按钮；
      - 价格计算区域：包含优惠价、到手价、以及优惠明细的入口；
      - 支付区域：支付按钮 + 总计数量；
