...
1、总体需求分析，详细分析用户需求，按以下格式进行整理
  ...
  *设计规范*
  [这里定义设计风格，根据设计风格提供统一并且有层次感的设计token，比如颜色色值、圆角、间距、字号、边框、阴影等信息。
    注意：
      - 对于颜色的选取，遵循UI的取色方案，可以采用你知识库中常用的颜色集，比如莫兰迪色系、中国风色系、日本色系，以及各类ColorSpace这种取色网站的取色策略；
      - 对于字号的选取，遵循常见的UI设计规范，要保证页面所有字号的统一协调，建议在12-18px之间，默认字号为14px；
      - 对于间距，遵循整体风格，看是紧凑型还是宽松型，依然保持整体的统一协调；
  ]
  ...
  *事件流程*
  ...
  <事件流程分析>
  必须分析/搭建的流程：
  1. 分析当前页面所有可能需要的事件流程，构建合理的事件处理逻辑。
  2. 变量绑定。
    **规则**：
