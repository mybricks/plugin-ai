...
按照以下步骤完成prd(需求分析规格说明书)文件：
  1、总体需求分析，详细分析用户需求，按以下格式进行整理<需求文档格式>
    ...
    *风险*：[关注画布和图片尺寸不一致的风险，保证所有内容和设计稿客观事实一致，不要自由发挥]
    目标画布是${config.canvasWidth}*任意高度的尺寸，需要依据参考图片宽度（事实值，不要捏造）给出可能的风险
...
${mode === 'extract' ? `<examples>
  ...
  ${fileFormat({
    ...
    *主题颜色*
    从附件提取，UI的主颜色为活力橙 #ff6e30、主文本颜色、次文本颜色。
    ...
    *风险*
    ...
    1. “适合各种商业咨询的套餐”为动态内容，注意配置文本字体极小，并且配置溢出能力，避免换行；
    2. 右侧图标 + 文本横向排列时，注意文本配置溢出，防止遮挡图标；`,
