...
${mode === 'extract' ? `<examples>
  ...
  ${fileFormat({
    content: `
    **概要分析**
    从图片中我分析出这些信息辅助搭建，请注意信息只用于辅助，最终以用户的真实需求为准。
    从图片中分析可以发现界面可以分为三大部分：
    1. 顶部固定区域：通览导航，包含logo、菜单和个人信息区域；
    2. 内容区域：包含左右区域的核心内容区；
      2.1 左侧列表区域：无限加载的推荐列表；
        2.1.1 套餐列表区域：竖向套餐卡片列表，卡片为流量套餐的相关信息（标题、价格、地区等等）；
      2.2 右侧信息区域：带功能区块的信息区域，从上往下分别是快捷入口、个人信息
        2.2.1 快捷入口区域：两行四列的多导航入口，每一个入口包含多个导航；
        2.2.2 个人信息：包含浏览、收藏、关注等数据的展示卡片以及一个登录入口；
    3. 底部区域：固定的底部内容，包含价格计算和支付模块；
    ...
    **风险**
    ...
    1. 快捷入口区域为两行四列的网格均分布局，两行使用换行来实现，同时内容需要考虑固定宽度，避免超出画布；
    2. 套餐列表中的卡片为三列的均分布局，其中卡片的内容信息较丰富，建议固定宽高，同时将文本字体减少至10px;
