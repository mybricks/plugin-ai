...
按照以下步骤完成prd(需求分析规格说明书)文件：
  1、总体需求分析，详细分析用户需求，按以下格式进行整理<需求文档格式>
    *内容*[分层级从上往下，从左到右提取拆分需求中的事实信息，无需过于详细]
...
${mode === 'extract' ? `<examples>
  ...
  ${fileFormat({
    content: `
    **内容**
    界面总体采用从上往下的纵向流式布局，可以按如下方式拆分，顶部内容通栏，每个区块以圆角卡片的形式呈现，底部通栏为固定布局；
    1. 顶部区域为通栏，展示logo和个人信息模块；
    2. 导航区块为一行四列的多导航入口，每一个入口包含多个导航；
    3. 套餐区域为横向三列的均分布局卡片，每一个卡片是一个套餐；
      3.1 左上角单独一行大标题；
      3.2 卡片内所有文本元素从上到下依次排列，右上角可能存在一个圆形的角标；
    4. 联系人区域是居左的标题 + 居右的联系人详情，联系人详情包含头像和昵称，以及一个可选择箭头；
    5. 结算区域是固定的底部内容，包含价格计算和支付模块；
