...
${mode === 'extract' ? `<examples>
  ...
  ${fileFormat({
    ...
    ## 事件流程
    1. 页面初始化加载事件
      - 调用接口获取推荐商品列表数据 → 渲染到中间内容区域的推荐卡片列表
      - 调用接口获取热榜数据 → 渲染到右侧推荐热榜区域
      - 调用接口获取用户个人信息 → 渲染到右侧个人信息区域的数据卡片
      - 调用接口获取积分表格数据 → 渲染到右侧积分表格区域
      - 调用接口获取公告信息 → 渲染到顶部公告区域
    
    2. 交互事件分析
      2.1 顶部区域
        - 公告区域：点击公告可查看详情（可能弹窗或跳转）
        - 退出登录按钮：点击触发退出登录流程，清除用户状态并跳转到登录页
    
      2.2 中间内容区域
        - 筛选区域：
          *分类标签点击*：点击地区/物流/评分标签时，筛选条件变化，重新调用商品列表接口获取筛选后的数据
          *搜索按钮点击*：点击后触发搜索，调用搜索接口并刷新商品列表
        - 推荐卡片区域：
          *加购按钮点击*：点击后将对应商品加入购物车，调用加购接口，成功后更新底部区域的总计数量
          *卡片点击*：点击卡片跳转到商品详情页
    
      2.3 右侧信息栏区域
        - 推荐热榜：
          *换一换按钮点击*：点击后重新调用热榜接口获取新的热榜数据
          *热榜项点击*：点击热榜项跳转到对应商品或搜索页
        - 个人信息区域：
          *登录按钮点击*：点击后弹出登录弹窗或跳转到登录页
          *数据卡片点击*：点击可能跳转到对应的浏览记录、收藏夹等页面
        - 积分表格：
          *操作列按钮点击*：如查看详情、兑换等操作，触发相应业务逻辑
    
      2.4 底部区域
        - 优惠明细入口点击：点击展开/弹窗显示优惠详情
        - 支付按钮点击：点击后触发支付流程，调用支付接口
    ...
    4. 状态管理需求
      - 用户登录状态：控制个人信息区域的显示内容
      - 购物车商品数量：底部区域和可能的其他位置需要共享
      - 当前筛选条件：筛选区域和商品列表区域需要同步
...
const eventPrompts = `*数据需求*
  [这里分析页面中需要动态数据支撑的原子组件和业务组件]
  <示例>
  - 原子组件
    - 列表类组件
    - 表格类组件
  ...
  *事件流程*
  ...
  - **页面初始化事件**：
    - 页面加载时自动调用接口获取初始数据
    - 用户信息获取：调用用户信息接口 → 更新用户信息组件显示
    - 列表数据获取：调用列表接口 → 更新列表组件数据
  
  - **用户交互事件**：
    - 按钮类
      - 查询按钮点击，调用查询接口，更新数据，刷新组件
      - 删除按钮，询问是否确认删除，调用删除接口，调用查询接口，更新数据，刷新组件
      - 链接按钮，点击跳转页面

  - **数据联动事件**：
    - 下拉框联动：省市区三级联动，上级选择变化时更新下级选项
    - 表单验证：实时验证输入内容，显示错误提示
    - 状态更新：操作成功后更新相关组件状态显示
  </示例>`