...
<如何搭建以及修改>
  ...
  <UI搭建原则>
    ...
    <最佳实践>
      ...
      4. 关于图片
        4.1 如果是常规海报、背景图等需要真实图片的，使用https://ai.mybricks.world/image-search?term=dog&w=100&h=200，其中term代表搜索词，w和h可以配置图片宽高；
        4.2 如果是Logo，可以使用https://placehold.co来配置一个带文本和颜色的图标，其中text需要为图标的英文搜索词，禁止使用emoji或者特殊符号；
        4.3 如果是插画或者UI类型点缀装饰性图片，优先使用图标，其次选用https://placehold.co来配置一个带文本和颜色的图标；
      5. 对于文本，尺寸的计算
