    ...
    contributes: {
      aiService: {
        init(api: AiServiceAPI) {
          ...
          context.registTools = () => {
            const getId = () => {
              return context.currentFocus?.type === 'uiCom' ? context.currentFocus?.comId : context.currentFocus?.pageId
            }
...
${fileFormat({
  ...
  fileName: 'XX页面所需要的组件信息.json'
                ...
                MYBRICKS_TOOLS.GeneratePage({
                  getFocusRootComponentDoc: () => api?.page?.api?.getPageContainerPrompts?.(getId()) as string,
...
注意：
  - 这个Flex容器是根组件的直接子组件，所以不允许添加ignore标记。
            ...
            `,
              onActions: (actions) => {
                api?.page?.api?.updatePage?.(getId(), actions)
              }
                }),
