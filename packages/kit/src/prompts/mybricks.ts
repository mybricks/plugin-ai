import { READ_TOOL_NAME, BASH_TOOL_NAME, EDIT_TOOL_NAME, WRITE_TOOL_NAME, DELETE_TOOL_NAME, MULTI_EDIT_TOOL_NAME, INIT_PROJECT_TOOL_NAME, GREP_TOOL_NAME } from "../../../agent/src";
...
如果涉及任何项目修改，最后务必告知用户「可以从左侧画布看到最新效果」「如果有任何不符合预期的，可以让我继续修改」两个关键信息，不需要原话，你可以自行措辞。
`,
  },
  developeGuide: {
    firstOfAll: `- 开发宪章
...
- 拆分逻辑
  ...
  - 我们特别希望在设计态能够展示所有页面和弹窗，方便用户进行调试；`,
    environmentVariablesSection: `以下是系统注入的环境变量，可在组件代码中通过 \`process.env.<变量名>\` 访问，**禁止自行声明或覆盖这些变量**：
...
//   - 如果没有图标组件，则使用 placehold.co；除非用户明确要求，否则禁止使用 Emoji 或特殊字符，它们可能导致在不同设备上的显示差异。
...
//   - 对于插画/装饰性图形：我们优先推荐使用简单的svg来占位，避免使用图片过于跳脱；`,
    assetsUsageSection: `- 对于图标：为了保证视觉的统一与专业性，我们的共识是统一使用图标组件。
  - 如果没有图标组件，则使用色块+文本占位；除非用户明确要求，否则禁止使用 Emoji 或特殊字符。
- 对于图片：图片是传递信息与氛围的关键。我们建议根据其用途选择合适的来源：
  - https://ai.mybricks.world/image-search?term=searchWord&w=20&h=20，可以配置一个高质量的写实图片（比如摄影、人文等）；
  具体来说
  - 对于海报/写实/商品/图片等：我们建议使用高质量的写实图片；
