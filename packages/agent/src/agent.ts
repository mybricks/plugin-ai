...
export class Agent {
  ...
  async requestAI(params: RequestAIOptions): Promise<void> {
    await this.ensureHistoryReady();
    const { turnId: requestedTurnId, message, displayMessage, modelMessage, attachments, mode = AgentModeEnum.Build, ...rest } = params;
...
1. 根据本轮的模型输出和操作内容，在必要条件充足时要生成「下一步输入建议选项」。
必要条件：只有在 本轮 *需求出现异常未完成情况* 且 *没有告知用户未完成的原因* 时，输出下一步输入建议选项。
否则，不需要输出这个选项。

输出不合理的选项会极大降低用户对你信任，必须谨慎输出。

<下下一步输入建议选项生成规则>
建议的内容格式为一句话 + 3个及以下的选项指令，内容用 <ask></ask> 标签包裹。

注意：所有建议需要和本轮对话有高度相关性。
只能基于确定的事实和能力，提供有事实依据的选项，确保选项能够完成，而不是输出一个不能完成的选项。

注意：所有建议需要从你的能力视角出发，而不是用户，这个建议选项需要你自己来完成，禁止给出需要用户操作的选项。

下方两个要点必须同时遵循：
1. 基于事实：基于已使用过的能力判断，不能杜撰没有使用过的能力，这会导致输出的建议是不可完成的；
2. 易于理解：option 的内容必须清晰，必须是一个描述清晰的需求，这个指令会作为下一步的指令给到用户；

重要：宁可不生成建议选项，也不要提供不合理/不可完成的建议！
<反模式选项>
不合理选项：
比如：从来没提到过「导出」功能，你却建议导出XX；
比如：从来没提过可以「刷新」，你却建议刷新；

无法完成选项：
比如：上一步要求用户填写一个表单，你的建议是提交这个表单，这是需要用户手动操作的内容，你越界了，你无法代完成这个提交操作；
