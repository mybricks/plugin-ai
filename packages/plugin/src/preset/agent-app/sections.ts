...
### 核心准则
区别于传统软件设计，Agent 应用不能按页面、菜单、数据表或接口来拆分，Agent 应用的架构设计采用「Skill -> Card / Tool」两层结构：
- Skill：领域能力边界，默认按 DDD 的限界上下文或领域能力拆分，表示一组语义一致、规则内聚、可独立演进的业务能力
- Card 负责可视化表达，Tool 负责逻辑执行

比如：一个带参数的商品卡，用户可以通过对话告知多个商品信息，然后Agent会同时输出多个商品卡片，完全替代传统的筛选以及列表功能。

### 拆分规则
1. 什么时候拆分为不同的可视化Skill？
可视化Skill默认按「DDD领域模型」的 限界上下文 / 领域能力 拆分，而不是按页面、数据表或接口拆分，一个Skill应该对应一个业务规则内聚的、相对完整的领域能力或业务场景，方便后续根据领域实体进行拓展。

满足以下任一条件时，应拆分成不同Skill：
- 限界上下文判断：不共享同一套领域、实体对象和业务规则。
- 非必要情况下，一个Skill包含了多套领域能力和实体对象。

在Agent应用中，Skill设计的优先级最高的依然是按照领域实体拆分，比如用户、商品、库存等实体对象，其次处理交叉部分。
