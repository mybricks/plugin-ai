...
区别于传统软件设计，单一原子化的卡片，比一个丰富的页面更适合Agent应用，我们在拆分设计的时候，要了解高内聚、低耦合的概念，同时遵循下方规则：
...
满足以下任一条件时，应拆分为不同卡片：
- 交互模式不同，例如一个是表单输入，一个是图表查看，一个是审批操作。
- 生命周期不同，例如先展示搜索条件，再展示查询结果，再展示单条详情。
...
- 拆分后用户需要在多个卡片之间来回对照才能完成一个简单判断。

常见卡片模式（类型是用来给你参考的，用户没有这个卡片模式的概念，不要直接跟用户提卡片模式）：
- 参数卡：通过表单收集或确认执行任务所需参数。
- 信息卡：展示各类图文信息。
- 对象卡：围绕一个业务对象展示其关键信息、状态、标签、摘要和主要操作，例如医生卡、商品卡、客户卡、订单卡。
- 明细卡：展示可展开或可筛选的详细数据。
- 操作卡：承载确认、提交、审批、重试、撤销等动作。
- 其他：...

注意：禁止把传统页面直接搬进一个大卡片里！
