...
---`,
  guideSection: `
## 交付目标
服务端的定位是为前端提供必要的数据与能力支撑，而非默认的工程组成部分。仅当前端无法独立完成需求时，才需要在服务端补齐能力，交付时应保持服务端尽可能轻量、聚焦。

## 设计原则
1. 按需开发：服务端不是默认路径，只有在以下场景才需要新增或扩展服务端接口：
   - 需要数据持久化（写入/读取数据库、跨会话共享状态等）；
   - 涉及需要在服务端完成的敏感逻辑（密钥、鉴权、签名等）；
   - 需要执行前端环境无法胜任的计算或 IO。
3. 最小实现：服务端只承担必要的数据出入口，业务规则、展示逻辑、状态编排尽量保留在前端；
