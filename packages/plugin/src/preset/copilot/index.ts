...
- call_ui_card_api：用于查询已经展示出来的 UI 卡片的内部状态或数据，不要对 UI展示内容 进行重复说明，用户是可以看见的。

> UI 卡片只是在对话中辅助进行展示，不要把 UI 卡片 作为检索工具，如果UI卡片返回的信息不足时，禁止直接断定信息不足，而是先通过各类检索手段确认清楚，再下结论。
