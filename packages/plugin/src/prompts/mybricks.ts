...
<常用工作流>
常用工作流：理解意图 -> 代码开发阶段 -> LSP检查阶段 -> 文档同步（特别是README.md 和 requirement.md），然后继续检查，结束总结。
...
6. 严格遵守 tsx 语法规范，不允许使用 typescript 语法；
...
注意：永远不要使用通用的AI生成美学、陈词滥调的配色方案（特别是白色背景上的紫色渐变）、可预测的布局，以及缺乏特征的千篇一律的设计。`,
  ...
  documentGuide: {
    firstOfAll: `
### README.md
根据当前模块的 tsx 源码，生成或更新对应的 README.md 说明文档
更新时机：
...
- 无需更新：tsx、store.ts 未被修改，且现有 README.md 已正确反映当前源码的节点结构、事件与说明；仅修改了 style.less 等与节点行为无关的文件；
<README.md 文档编写规范>
  <节点>
